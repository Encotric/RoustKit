...
while (("$#")); do
	...
	-d | --distribution)
		...
		;;
	-s | --seek)
		# Sets the size of the file system, default 3G
		SEEK=$(($2 - 1))
		shift 2
		;;
	-n | --name)
...
pushd ${MNT_BASE} >/dev/null || exit 255
...
dd if=/dev/zero of="${ROOTFS_DST_DIR}/$ROOTFS_NAME" bs=1M seek=$SEEK count=1 &>/dev/null
sudo mkfs.ext4 -F "${ROOTFS_DST_DIR}/$ROOTFS_NAME" &>/dev/null
sudo mkdir -p /mnt/$MNT >/dev/null
sudo mount -o loop "${ROOTFS_DST_DIR}/$ROOTFS_NAME" /mnt/$MNT >/dev/null
